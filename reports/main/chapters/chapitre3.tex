...
En matière de \textbf{détection de conflits}, le moteur dédié valide systématiquement 7 règles de gestion lors de chaque modification du planning (voir \cref{ch3:regles_gestion}). Dupliqué côté client pour un retour instantané et exécuté côté serveur pour préserver l'intégrité lors de la sauvegarde, cette architecture double-validation combine réactivité et fiabilité.
...
\begin{projectbox}{Règles de Gestion du Planning}
    \begin{description}
        ...
        \item[RG6 (Pause)] Un intervalle minimum (\textit{breakDuration}) doit être respecté entre deux soutenances dans une même salle.
        \item[RG7 (Disponibilité)] Aucune soutenance ne peut être planifiée sur un créneau déclaré comme indisponible par un membre du jury.
    \end{description}
\end{projectbox}

Ces 7 règles sont implémentées dans deux moteurs distincts mais fonctionnellement identiques : un moteur Java côté serveur (\texttt{ConflictDetectionService}) qui valide lors de la sauvegarde, et un moteur TypeScript côté client (\texttt{conflict-engine.ts}) qui fournit un retour instantané lors des interactions dans le Defense Designer.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lXccc}
        ...
        RG6            & Intervalle de pause      & \checkmark    & \checkmark          & error             \\
        RG7            & Indisponibilité          & \checkmark    & \checkmark          & error             \\
        \bottomrule
    \end{tabularx}
...
L'algorithme de détection des conflits, moteur central du système, est modélisé par le diagramme d'activité de la \cref{fig:act_conflict}. Chaque vérification est une étape indépendante, permettant d'identifier l'ensemble des violations en un seul passage. L'algorithme exécute les 8 règles de gestion en séquence et collecte toutes les violations détectées avant de retourner le résultat au frontend.

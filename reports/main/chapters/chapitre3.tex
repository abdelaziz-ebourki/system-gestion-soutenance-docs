...
La \textbf{réactivité} constitue un besoin fondamental pour le Defense Designer : le retour sur conflit lors du drag-and-drop doit être instantané (latence $< 100$ms) grâce à la validation locale via le moteur de conflits TypeScript. L'état du planning est maintenu dans la mémoire du navigateur, sans aucun appel réseau pendant les opérations de glisser-déposer.
...
Côté \textbf{disponibilité}, une architecture stateless du backend permet un déploiement horizontal scalable. L'API REST ne maintient pas d'état de session côté serveur : chaque requête est autonome via le token JWT, ce qui facilite tests et débogage.

L'\textbf{ergonomie} s'appuie sur une interface moderne et responsive (Tailwind CSS 4, shadcn/ui). La navigation est contextualisée par rôle avec une sidebar adaptative, et le thème clair/sombre avec persistance du choix améliore le confort lors de sessions prolongées.

La \textbf{maintenabilité} résulte d'une architecture en couches stricte (Contrôleur $\rightarrow$ Service $\rightarrow$ Repository $\rightarrow$ Entité) avec séparation claire des responsabilités. Les DTOs isolent les couches API et MapStruct gère le mapping objet, protégeant le modèle interne des variations de l'interface exposée.

Enfin, la \textbf{qualité} est mesurée par une couverture minimale de 85\% (lignes) et 70\% (branches) via JaCoCo, complétée par un linting automatisé avec ESLint (TypeScript), et Spotless, Checkstyle, PMD et SpotBugs (Java) intégré dans CI.
...
\begin{projectbox}{Règles de Gestion du Planning}
    \begin{description}
        ...
        \item[RG3 (Période)] Toutes les soutenances d'une session doivent être planifiées entre la \textit{startDate} et la \textit{endDate} de ladite session.
...
\Cref{fig:seq_planning} modélise le flux de planification interactive via le Defense Designer. Ce processus repose sur un cycle de validation itératif où le frontend et le backend collaborent pour préserver l'absence de conflits :
...
\Cref{fig:seq_auth} modélise le flux d'authentification. L'utilisateur soumet ses identifiants via le formulaire de connexion, le frontend envoie une requête POST à \texttt{/api/auth/login}, le serveur valide les credentials (vérification du hash BCrypt et du statut du compte), puis retourne un token JWT via un cookie \texttt{httpOnly} sécurisé. Le navigateur gère alors automatiquement l'envoi de ce cookie lors des requêtes suivantes, et le frontend redirige l'utilisateur vers la page correspondant à son rôle.

% ═══════════════════════════════════════════════════════════════════════════════
%  CONCLUSION ET BIBLIOGRAPHIE
% ═══════════════════════════════════════════════════════════════════════════════
\newpage
\section*{Conclusion Générale}
\addcontentsline{toc}{section}{Conclusion Générale}
\label{sec:conclusion_generale}

Ce projet de fin d'études a permis la conception et le développement d'une solution complète pour la gestion des soutenances et des jurys au sein d'un établissement universitaire. Partant des constats d'inefficacité des processus manuels existants -- tableurs Excel, échanges d'e-mails, documents papier -- nous avons développé une application web moderne et intégrée, articulée autour d'une architecture Spring Boot (backend Java 17) et React (frontend TypeScript 6, Tailwind 4), répondant aux problématiques concrètes de coordination, d'automatisation des flux documentaires et de gestion des contraintes liées à l'organisation des soutenances universitaires.

Les objectifs initiaux ont été atteints avec succès. L'automatisation de la planification, via un designer de défenses interactif avec détection automatique de conflits basée sur huit règles de gestion (RG1--RG8), a remplacé le processus manuel chronophage et sujet aux erreurs. L'intégration d'un moteur d'auto-génération de planning a permis de réduire significativement le temps de constitution des plannings de soutenance. La production documentaire automatisée -- convocations, fiches d'évaluation, procès-verbaux -- élimine la saisie manuelle répétitive et garantit l'uniformité des formats. Le module d'évaluation, avec calcul automatique des moyennes pondérées et gestion formelle des délibérations, assure la fiabilité et la traçabilité des résultats académiques. Le contrôle granulaire des accès, basé sur quatre rôles (administrateur, coordinateur, enseignant, étudiant) avec JWT et tokens d'activation, garantit la sécurité des données sensibles.

Le système a été validé via une stratégie de test multi-couche comprenant 86 classes de test backend (JUnit 5 + Mockito) avec 85\% de couverture sur les lignes et 70\% sur les branches, 91 fichiers de test frontend (Vitest + React Testing Library) et environ 65 scénarios de test de bout en bout (Playwright) exécutés contre l'API Spring Boot réelle et la base de données H2. Les outils de qualité statique (JaCoCo, Checkstyle, PMD, SpotBugs, ESLint, Prettier) intégrés au pipeline Maven et aux hooks Husky garantissent la pérennité de la qualité du code. Ce système constitue un levier d'amélioration opérationnelle significatif pour les établissements universitaires, permettant de réduire la charge administrative tout en augmentant la fiabilité et la traçabilité des processus organisationnels.

Les perspectives d'évolution sont nombreuses et pertinentes. L'intégration d'un moteur de rendu PDF côté serveur (via Puppeteer ou iText) permettrait d'automatiser la génération et l'envoi des convocations par email, éliminant ainsi le besoin d'impression manuelle. Le développement d'une application mobile dédiée offrirait des notifications en temps réel via les services push natifs, améliorant la réactivité des enseignants et des étudiants face aux changements de planning. L'optimisation des algorithmes d'auto-génération du planning, en remplaçant l'approche gloutone actuelle par des techniques de programmation par contraintes ou de recuit simulé, permettrait de prendre en compte des contraintes de préférence plus fines des enseignants et d'optimiser l'occupation des salles. Enfin, l'intégration de WebSocket pour les mises à jour en temps réel et le développement d'un mode hors ligne progressive (PWA) constitueraient des améliorations significatives de l'expérience utilisateur.

\newpage
\begin{thebibliography}{99}
\addcontentsline{toc}{section}{Bibliographie}

\bibitem{spring}
Spring Boot Documentation, version 3.4.6,
\url{https://docs.spring.io/spring-boot/docs/3.4.6/reference/html/}

\bibitem{spring_data}
Spring Data JPA Documentation,
\url{https://docs.spring.io/spring-data/jpa/docs/current/reference/html/}

\bibitem{spring_security}
Spring Security Documentation,
\url{https://docs.spring.io/spring-security/reference/}

\bibitem{hibernate}
Hibernate ORM Documentation, version 6.6,
\url{https://docs.hibernate.org/hibernate/6.6/htmlguide/}

\bibitem{java}
Java SE 17 Documentation,
\url{https://docs.oracle.com/en/java/javase/17/}

\bibitem{react}
React Documentation, version 19,
\url{https://react.dev/reference/react}

\bibitem{react_router}
React Router Documentation, version 7,
\url{https://reactrouter.com/}

\bibitem{tanstack}
TanStack Query Documentation, version 5,
\url{https://tanstack.com/query/latest}

\bibitem{shadcn}
Shadcn/ui Documentation,
\url{https://ui.shadcn.com/}

\bibitem{tailwind}
Tailwind CSS Documentation, version 4,
\url{https://tailwindcss.com/docs}

\bibitem{vite}
Vite Documentation, version 8,
\url{https://vite.dev/guide/}

\bibitem{typescript}
TypeScript Documentation, version 6,
\url{https://www.typescriptlang.org/docs/}

\bibitem{playwright}
Playwright Documentation, version 1.52,
\url{https://playwright.dev/docs/intro}

\bibitem{vitest}
Vitest Documentation, version 4.1,
\url{https://vitest.dev/guide/}

\bibitem{msw}
Mock Service Worker (MSW) Documentation, version 3,
\url{https://mswjs.io/}

\bibitem{jwt}
RFC 7519 -- JSON Web Token (JWT),
\url{https://datatracker.ietf.org/doc/html/rfc7519}

\bibitem{mysql}
MySQL 8.0 Reference Manual,
\url{https://dev.mysql.com/doc/refman/8.0/en/}

\bibitem{h2}
H2 Database Engine Documentation,
\url{https://h2database.com/html/main.html}

\bibitem{git}
Pro Git Book, version 2,
\url{https://git-scm.com/book/}

\bibitem{plantuml}
PlantUML Documentation,
\url{https://plantuml.com/}

\bibitem{uml}
Unified Modeling Language (UML) Specification, version 2.5.1,
\url{https://www.omg.org/spec/UML/2.5.1}

\bibitem{agile}
Manifesto for Agile Software Development,
\url{https://agilemanifesto.org/}

\bibitem{lmd}
Organisation LMD -- Ministère de l'Enseignement Supérieur du Maroc,
\url{https://www.gov.ma/}

\end{thebibliography}

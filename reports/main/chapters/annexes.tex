...
\section{Liste complète des endpoints API}
\label{annex:api_endpoints}

L'API REST expose 124 endpoints organisés par module. Le préfixe commun est \texttt{/api}.
...
\begin{enumerate}
    \item Se connecter avec les identifiants administrateur fournis (\texttt{admin@fsbm.ma}).
    \item Le \textit{SetupGuard} redirige automatiquement vers la page de configuration (\texttt{/admin/config}).
    \item Configurer les paramètres de soutenance : horaires de début/fin, durée des soutenances, intervalle de pause.
    \item Créer les départements, les filières, les niveaux et les salles.
    \item Créer les comptes des coordinateurs et enseignants (individuellement ou en masse via l'import CSV).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Créer un groupe ou rejoindre un groupe existant via le code d'invitation.
    \item Consulter la composition du groupe.
\end{enumerate}
...
\begin{itemize}
    ...
    \item La convocation peut être téléchargée directement depuis l'application (\texttt{/student/convocations}).
\end{itemize}

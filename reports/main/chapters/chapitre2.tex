...
Ce chapitre présente une analyse approfondie du système actuel d'organisation des soutenances au sein de la faculté, identifie ses lacunes et compare les solutions existantes sur le marché. Il détaille ensuite les choix technologiques retenus pour le développement de notre application. Enfin, l'architecture générale du système est présentée ; la stratégie de déploiement et l'organisation du code source sont détaillées aux chapitres suivants (cf. \cref{ch3:architecture_logicielle,ch4:architecture}).
...
En phase de développement et de test, \textbf{H2 Database} est utilisé en mode mémoire. Cette base de données embarquée ne nécessite aucune installation, démarre instantanément et permet de réinitialiser l'état à chaque redémarrage. Le seed data initialise automatiquement la base avec des données de démonstration (\metric{SeedEtudiants} étudiants, \metric{SeedEnseignants} enseignants, \metric{SeedProjets} projets, \metric{SeedSoutenances} soutenances, etc.), ce qui facilite les tests fonctionnels et le développement.
...
\textbf{Git et GitHub} constituent le socle de la gestion de version et de la collaboration. Le projet est organisé en dépôts multiples, chacun doté de son propre historique de commits et de sa configuration \gls{cicd}. La stratégie de branching suit un modèle simplifié (branche \texttt{main} stable, branches \texttt{feature/*} et \texttt{fix/*}). Les conventions sémantiques de commit et la gestion des issues sont détaillées au chapitre~4 (cf. \cref{ch4:git_conventions}).
...
Des hooks de pré-commit (Husky, lint-staged) exécutent le linting et le formatage avant chaque commit, formant une barrière de qualité automatisée. Couplés à Vitest et JaCoCo, ces outils constituent un filet de sécurité complet.

\subsection{Architecture générale retenue}
\label{ch2:architecture}

L'application adopte une architecture web moderne en trois couches (frontend React, backend Spring Boot, base de données), conçue selon une approche \textit{\acrshort{api}-first} où chaque couche communique via un contrat \acrshort{rest} formalisé et documenté via Swagger/OpenAPI. Les endpoints respectent les conventions \acrshort{rest} avec des verbes \gls{http} standards et des réponses \gls{json} structurées. L'architecture complète, l'organisation multi-dépôts et la stratégie de déploiement sont détaillées aux chapitres suivants (cf. \cref{ch3:architecture_logicielle,ch4:architecture}).

\subsection{Conclusion}
\label{ch2:conclusion}

Ce chapitre a dressé un état des lieux complet du système actuel d'organisation des soutenances, mettant en évidence ses limites opérationnelles -- charge de planification concentrée sur le coordinateur, tâches répétitives de saisie, absence de détection automatique de conflits. L'analyse des solutions existantes -- systèmes ERP, outils génériques et projets open source -- a confirmé l'absence d'un outil répondant simultanément à tous les besoins du contexte universitaire marocain. Les choix technologiques -- React 19, Spring Boot 3.4.6, \acrshort{mysql}, Docker -- ont été justifiés par une évaluation approfondie des alternatives, et la chaîne de qualité de code a été présentée comme garantie de maintenabilité. L'architecture \acrshort{api}-first et l'organisation du code constituent les fondations techniques du système. Le chapitre suivant détaillera l'analyse des besoins fonctionnels et la conception complète du système.
